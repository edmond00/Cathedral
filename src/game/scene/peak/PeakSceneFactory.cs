...
public class PeakSceneFactory : SceneFactory
{
    ...
    protected override void BuildSections(Random rng, int locationId, Scene scene)
    {
        ...
        // The high ground sees the whole place. The summit approach is the top of the cliff, so
        ...
        if (_summitApproach != null)
            AddLandscapes(scene, _summitApproach, scene.AllAreas);
}
