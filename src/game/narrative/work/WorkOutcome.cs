...
/// <summary>
/// The per-skill result of working a job for a number of days: how much XP it earned, whether the
...
///
/// XP is front-loaded: the first (most-defining) skill earns 1 XP per 30 days, the second 1 per
/// 90 days, the third 1 per 150 days. Known skills accrue that XP (levelling up as the pineal
/// threshold is crossed, capped at max level). An unknown skill is learned at level 1 only if its XP
/// for the stint reaches the threshold in one go; otherwise nothing is learned for it.
/// </summary>
public sealed class WorkOutcome
{
    /// <summary>Days of work that earn the most-defining skill a single XP point.</summary>
    private const int DaysPerXp = 30;
    ...
    /// <summary>XP the skill at ordinal <paramref name="index"/> (0..2) earns over <paramref name="days"/>.</summary>
    public static int XpForSkill(int index, int days) => index switch
    {
        0 => days / DaysPerXp,
        1 => days / (DaysPerXp * 3),
        _ => days / (DaysPerXp * 5),
    ...
    private static int CoinsFor(Job job, int days)
        => job.DaysPerCoin <= 0f ? 0 : (int)Math.Floor(days / job.DaysPerCoin);

    /// <summary>Computes the outcome for the UI preview without changing any game state.</summary>
    public static WorkOutcome Preview(Job job, int days, PartyMember member)
        => Build(job, days, member, apply: false, party: null);

    /// <summary>Applies the outcome: credits coins, awards XP to known skills, learns eligible unknown ones.</summary>
    public static WorkOutcome Apply(Job job, int days, PartyMember member, Party party)
        => Build(job, days, member, apply: true, party: party);

    private static WorkOutcome Build(Job job, int days, PartyMember member, bool apply, Party? party)
    {
        int coins = CoinsFor(job, days);
