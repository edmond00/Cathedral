...
public class SnailTrailNode : NarrationNode
{
    ...
    public override List<KeywordInContext> NodeKeywordsInContext => new() { KeywordInContext.Parse("the glistening <slime> trail on a leaf"), KeywordInContext.Parse("the winding <trail> left across the bark"), KeywordInContext.Parse("the perfect <spiral> of an empty shell"), KeywordInContext.Parse("the iridescent <mucus> still wet underfoot") };
    ...
    public sealed class SnailShell : Item
    {
        ...
        public override List<KeywordInContext> OutcomeKeywordsInContext => new() { KeywordInContext.Parse("the final <whorl> of a spiral snail shell"), KeywordInContext.Parse("the pure white <calcium> of the empty shell") };
    }
    ...
    public sealed class SlimeTrail : Item
    {
        ...
        public override List<KeywordInContext> OutcomeKeywordsInContext => new() { KeywordInContext.Parse("the sticky <mucus> left by the passing snail"), KeywordInContext.Parse("a faint <iridescence> in the slime trail") };
    }
    ...
    public sealed class MossClump : Item
    {
        ...
        public override List<KeywordInContext> OutcomeKeywordsInContext => new() { KeywordInContext.Parse("a damp <bryophyte> tuft where the snails graze"), KeywordInContext.Parse("a soft green <tuft> of moss with trails across it") };
    }
}
