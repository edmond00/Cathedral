...
public static class NpcAudit
{
    ...
    public static string BuildReport()
    {
        ...
        sb.AppendLine();
        warnings.AddRange(CheckBeasts(sb));
    ...
    private static IEnumerable<string> CheckTraitCatalogue(StringBuilder sb)
    {
        ...
        foreach (var archetypeId in archetypeIds)
        {
            ...
            foreach (var trait in registry.ForArchetype(archetypeId))
            {
                ...
                // Same rule for the wound half, and the same exemption for global traits.
                foreach (var factory in trait.Wounds)
                {
                    var wound = factory();
                    if (!WoundRegistry.CanBeSufferedBy(wound, species.Value))
                        warnings.Add($"trait '{trait.TraitId}' ({archetypeId}, {species}) inflicts "
                                     + $"'{wound.WoundName}', which that anatomy does not own");
                }
            }
        }
    ...
    /// <summary>
    /// The same per-individual checks, run over every <b>non-human</b> archetype.
    ///
    /// <para>
    /// A separate pass because the sample above is <see cref="SpeakingArchetypes"/> — a hand-written
    /// list of humans — so no animal was ever generated by this audit at all. That is exactly where
    /// the anatomy checks have something to find: global personality traits are dealt to every
    /// anatomy, and eight of the sixty carry a <i>human</i> wound, so roughly one beast in four was
    /// given a broken knee or a missing finger. Inert in play, and fatal at load — see
    /// <see cref="WoundRegistry.CanBeSufferedBy(Wound, AnatomyType)"/>.
    /// </para>
    ///
    /// <para>Discovered by reflection rather than listed, so a new animal is covered the day it is
    /// written; the human list stays hand-written because it also drives the social-standing table.</para>
    /// </summary>
    private static IEnumerable<string> CheckBeasts(StringBuilder sb)
    {
        var warnings = new List<string>();

        sb.AppendLine("── Beasts ────────────────────────────────────────────────────────────");
        sb.AppendLine();
        sb.AppendLine($"{"archetype",-18}{"organs",9}{"skills",8}{"lvl",6}{"wounds",8}{"repeatable",12}");
        sb.AppendLine(new string('─', 61));

        foreach (var archetype in NonHumanArchetypes())
        {
            var organTotals = new List<int>();
            var skillCounts = new List<int>();
            var levels      = new List<int>();
            ...
            sb.AppendLine(
                $"{archetype.ArchetypeId,-18}{Range(organTotals),9}{Range(skillCounts),8}" +
                $"{Range(levels),6}{Range(woundCounts),8}{(repeatable ? "yes" : "NO"),12}");
        }

        return warnings.Distinct();
    }

    /// <summary>Every concrete archetype whose species is not human, one instance each.</summary>
    private static IEnumerable<NamedNpcArchetype> NonHumanArchetypes()
    ...
    private static IEnumerable<string> CheckIndividual(NpcEntity npc)
    {
        ...
        // A wound this body does not own. It penalises nothing (every Affects* query misses an organ
        // part the anatomy lacks), so it reads as a mere hit point off — but it is captured into the
        // save verbatim and PartyState.Rebuild refuses to restore it, which loses the whole run.
        foreach (var wound in body.Wounds)
            if (!WoundRegistry.CanBeSufferedBy(wound.Template, body))
                yield return $"{who} ({body.AnatomyType}): carries '{wound.WoundName}', which this "
                             + $"anatomy does not own (targets {wound.TargetKind} '{wound.TargetId}') "
                             + "— this makes a save holding them unloadable";
        ...
        // Expected against the pool actually available: a beast archetype has no trait pool of its
        // own, so it is dealt the global draw alone and a flat count would warn about every animal.
        int expectedTraits = PersonalityTraitRegistry.GlobalDraw
                             + Math.Min(PersonalityTraitRegistry.ArchetypeDraw,
                                        PersonalityTraitRegistry.Instance.ForArchetype(who).Count);
        if (npc.Text.TraitIds.Count != expectedTraits)
            yield return $"{who}: dealt {npc.Text.TraitIds.Count} traits, expected {expectedTraits}";
    }
